...
void rotate_and_render(SDL_Renderer* renderer, double angle,
                       SDL_Surface** master_surface, SDL_Window* window,
                       SDL_Texture** window_output);
void apply_grayscale(SDL_Renderer* renderer, SDL_Surface* master_surface,
                     SDL_Texture** window_output);
void blur_gaussian3(SDL_Renderer* renderer, SDL_Surface* master_surface,
                    SDL_Texture** window_output);
void binarize_otsu(SDL_Renderer* renderer, SDL_Surface* master_surface,
                   SDL_Texture** window_output);
void close_black3x3(SDL_Renderer* renderer, SDL_Surface* master_surface,
                    SDL_Texture** window_output, int iterations);
void median3x3(SDL_Renderer* renderer, SDL_Surface* master_surface,
               SDL_Texture** window_output);
double estimate_rotation_angle_projection_full(SDL_Surface** surface);
void repair_strokes(SDL_Renderer* renderer, SDL_Surface* master_surface,
                    SDL_Texture** window_output);
void binarize(SDL_Renderer* renderer, SDL_Surface* master_surface,
              SDL_Texture** window_output);
char* line_maker(int lineCount, const char* folder, NeuralNetwork* nn);
void grid_maker(Box*** gridLines, int** gridCount, int nbLinesGrid,
                SDL_Surface** master_surface, NeuralNetwork* nn);
char** word_maker(Box*** wordLines, int** wordCount, int nbLinesWord,
                  SDL_Surface** master_surface, NeuralNetwork* nn);
void solver_call_surface(SDL_Surface** surface, Box*** gridBoxes,
                         char** wordlist, int nbLinesWord, int* x1, int* y1,
                         int* x2, int* y2);
Coord (*solve(Box*** gridBoxes, char** wordlist, int nbLinesWord))[2];
void drawsolution(Coord (*to_draw)[2], SDL_Surface** master_surface,
                  Box*** gridBoxes, int* gridCount, int nbLinesGrid, int num);

// Loads an image file into the main surface and updates the window texture.
static int load_into_master(SDL_Renderer* renderer,
                            SDL_Surface** master_surface, SDL_Window* window,
                            SDL_Texture** window_output, const char* filename)
{
    if (!renderer || !master_surface || !window || !window_output ||
        !filename || !*filename)
    {
...
// Rotates the master surface by an angle and fills transparent pixels with
// white.
void RotateSurface(SDL_Surface** masterSurface, double alpha)
{
    ...
    if (src->format->format != SDL_PIXELFORMAT_RGBA32)
    {
        ...
        {
            fprintf(stderr,
                    "RotateSurface: SDL_ConvertSurfaceFormat failed: %s\n",
                    SDL_GetError());
            return;
        }
        must_free_work = 1;
...
// Cuts each box area from the surface and saves it as a BMP file.
void extract_boxes_to_bmp(SDL_Surface* master_surface, Box* boxes,
                          int box_count, const char* folder)
{
    ...
    for (int i = 0; i < box_count; i++)
    {
        ...
        if (b.y < 0)
            b.y = 0;
        if (b.x + b.w > master_surface->w)
            b.w = master_surface->w - b.x;
        if (b.y + b.h > master_surface->h)
            b.h = master_surface->h - b.y;
        if (b.w <= 0 || b.h <= 0)
            continue;

        SDL_Rect rect = {b.x, b.y, b.w, b.h};
        SDL_Surface* sub = SDL_CreateRGBSurfaceWithFormat(
            0, b.w, b.h, master_surface->format->BitsPerPixel,
        ...
        char filename[256];
        snprintf(filename, sizeof(filename), "%s/_box_%d.bmp",
                 folder ? folder : "images", i);
...
// Handles SDL events and runs commands from the command window.
int event_handler(SDL_Renderer* renderer, SDL_Surface** master_surface,
                  SDL_Window* window, SDL_Texture** window_output)
{
    SDL_Event current;
    ...
    while (SDL_PollEvent(&current))
    {
        ...
        {
            rotate_and_render(renderer, cmd.value, master_surface, window,
                              window_output);
        }
        ...
        {
            double alpha =
                estimate_rotation_angle_projection_full(master_surface);
            printf("autorotate(full): %.3f deg\n", alpha);
            rotate_and_render(renderer, alpha, master_surface, window,
                              window_output);
        }
        ...
        if (strcmp(cmd.operation, "boxes") == 0)
        {
            ...
            int* blob_sizes = NULL;
            Coord** blobs =
                find_blobs_rec(master_surface, &blob_count, &blob_sizes);
            ...
            separate_grid_word(testBox, testCount, gridBoxes, gridCount,
                               wordBoxes, wordCount, &nbLinesGrid,
                               &nbLinesWord);
            ...
            for (int i = 0; i < nbLinesGrid; i++)
                draw_boxes(master_surface, (*gridBoxes)[i], (*gridCount)[i], 0,
                           0, 255);

            for (int i = 0; i < nbLinesWord; i++)
                draw_boxes(master_surface, (*wordBoxes)[i], (*wordCount)[i],
                           255, 0, 0);

            if (*window_output)
                SDL_DestroyTexture(*window_output);
            *window_output =
                SDL_CreateTextureFromSurface(renderer, *master_surface);

            extract_boxes_to_bmp(*master_surface, testBox, testCount, "images");
        }
        if (strcmp(cmd.operation, "load") == 0)
        {
            const char* p = cmd.raw + 4;
            while (*p == ' ')
                p++;
            ...
            {
                printf("loading: %s\n", p);
                load_into_master(renderer, master_surface, window,
                                 window_output, p);
            }
        }
        ...
        if (strcmp(cmd.operation, "auto") == 0)
        {
            ...
            char** wordlist = word_maker(wordBoxes, wordCount, nbLinesWord,
                                         master_surface, nn);
            Coord(*first)[2] = solve(gridBoxes, wordlist, nbLinesWord);
            if (!first)
            {
                printf("solve failed (first)\n");
                break;
            }
            ...
            char** wordlist2 = word_maker(wordBoxes, wordCount, nbLinesWord,
                                          master_surface, nn2);
            Coord(*second)[2] = solve(gridBoxes, wordlist2, nbLinesWord);
            if (!second)
            {
                printf("solve failed (second)\n");
                free(first);
                break;
            }
            // printf("after solve2\n");
            ...
            for (int i = 0; i < nbLinesWord; i++)
            {
                if (first[i][0].x == 0 && first[i][0].y == 0 &&
                    first[i][1].x == 0 && first[i][1].y == 0)
                {
                    continue;
                }
                firstsize++;
            ...
            for (int i = 0; i < nbLinesWord; i++)
            {
                if (second[i][0].x == 0 && second[i][0].y == 0 &&
                    second[i][1].x == 0 && second[i][1].y == 0)
                {
                    continue;
                }
                secondsize++;
            ...
            {
                drawsolution(firstfix, master_surface, gridBoxes, *gridCount,
                             nbLinesGrid, firstsize);
            }
            else
                drawsolution(secondfix, master_surface, gridBoxes, *gridCount,
                             nbLinesGrid, secondsize);
            if (remove("grid.txt") != 0)
                // printf("remove\n");
        ...
        if (strcmp(cmd.operation, "learn") == 0)
        {
            ...
            if (sscanf(cmd.raw, "learn %255s %255s %d %lf %d", dataset, out,
                       &epochs, &lr, &hidden) == 5)
            {
                train(dataset, out, epochs, lr, hidden, cmdwindow_get_logger(),
                      cmdwindow_get_logger_userdata());
            }
            else
            {
                printf("usage: learn <dataset> <model_out> <epochs> <lr> "
                       "<hidden>\n");
            }
        }
    }
...
// Builds a string by predicting one character per saved box image.
char* line_maker(int lineCount, const char* folder, NeuralNetwork* nn)
{
...
// Creates grid.txt by extracting grid lines and running the NN on each cell.
void grid_maker(Box*** gridLines, int** gridCount, int nbLinesGrid,
                SDL_Surface** master_surface, NeuralNetwork* nn)
{
    for (int i = 0; i < nbLinesGrid; i++)
    {
        extract_boxes_to_bmp(*master_surface, (*gridLines)[i], (*gridCount)[i],
                             "images");
...
// Builds the word list by extracting each word line and predicting letters.
char** word_maker(Box*** wordLines, int** wordCount, int nbLinesWord,
                  SDL_Surface** master_surface, NeuralNetwork* nn)
{
    ...
    for (int i = 0; i < nbLinesWord; i++)
    {
        extract_boxes_to_bmp(*master_surface, (*wordLines)[i], (*wordCount)[i],
                             "images");
...
// Solves each word and draws it directly on the given surface.
void solver_call_surface(SDL_Surface** surface, Box*** gridBoxes,
                         char** wordlist, int nbLinesWord, int* x1, int* y1,
                         int* x2, int* y2)
{
...
// Calls the solver for each word and returns the list of found coordinates.
Coord (*solve(Box*** gridBoxes, char** wordlist, int nbLinesWord))[2]
{
    (void)gridBoxes;
    ...
    for (int i = 0; i < nbLinesWord; i++)
    {
        ...
        res[i][1].x = 0;
        res[i][1].y = 0;
        solver("grid.txt", wordlist[i], &res[i][0].x, &res[i][0].y,
               &res[i][1].x, &res[i][1].y);
    }
...
// Draws all solved words on the master surface using the grid boxes.
void drawsolution(Coord (*to_draw)[2], SDL_Surface** master_surface,
                  Box*** gridBoxes, int* gridCount, int nbLinesGrid, int num)
{
    for (int i = 0; i < num; i++)
...
// Runs the main loop to process events and keep rendering the texture.
void update(SDL_Renderer* renderer, SDL_Surface** master_surface,
...
// Initializes SDL, creates the window, and starts the app loop.
int main(void)
{
    TTF_Init();
    ...
    SDL_Surface* master_surface = SDL_CreateRGBSurfaceWithFormat(
...
// Rotates the surface, resizes the window, and updates the render texture.
void rotate_and_render(SDL_Renderer* renderer, double angle,
                       SDL_Surface** master_surface, SDL_Window* window,
                       SDL_Texture** window_output)
{
...
// Converts the surface to grayscale and refreshes the displayed texture.
void apply_grayscale(SDL_Renderer* renderer, SDL_Surface* master_surface,
                     SDL_Texture** window_output)
{
...
// Checks if a pixel is part of the foreground based on brightness.
static int is_foreground(SDL_Surface* s, Uint32 px, int foreground_is_black)
{
    Uint8 r, g, b, a;
...
// Detects if the drawing is mostly black or mostly white.
...
// Computes a projection score for a given rotation angle.
static double projection_score_fg(SDL_Surface* s, double a_rad, int step,
                                  int fg_black)
{
    ...
    for (int y = 0; y < h; y += step)
    {
        ...
        {
            if (!is_foreground(s, row[x], fg_black))
                continue;
            double xr = x * ca + y * sa;
            double yr = -x * sa + y * ca;
            ...
            if ((unsigned)ix < (unsigned)diag)
                hx[ix]++;
            if ((unsigned)iy < (unsigned)diag)
                hy[iy]++;
        }
    }
...
// Estimates the best rotation angle by maximizing projection score.
double estimate_rotation_angle_projection_full(SDL_Surface** surface)
{
    if (!surface || !*surface)
        return 0.0;
    ...
    {
        double sc =
            projection_score_fg(s, deg * M_PI / 180.0, STEP_PIXELS, fg_black);
        if (sc > best_score)
        {
            best_score = sc;
            best_deg = deg;
...
// Applies a simple 3x3 Gaussian blur to the surface.
void blur_gaussian3(SDL_Renderer* renderer, SDL_Surface* master_surface,
                    SDL_Texture** window_output)
{
    if (!renderer || !master_surface || !window_output)
...
#define CLAMP(v, lo, hi) ((v) < (lo) ? (lo) : ((v) > (hi) ? (hi) : (v)))
    ...
    for (int y = 0; y < h; y++)
    {
        for (int x = 0; x < w; x++)
        {
            ...
            SDL_GetRGBA(src[y0 * pitch32 + x0], fmt, &r, &g, &b, &a);
            ar += 1 * r;
            ag += 1 * g;
            ab += 1 * b;
            aa += 1 * a;
            SDL_GetRGBA(src[y0 * pitch32 + x1], fmt, &r, &g, &b, &a);
            ar += 2 * r;
            ag += 2 * g;
            ab += 2 * b;
            aa += 2 * a;
            SDL_GetRGBA(src[y0 * pitch32 + x2], fmt, &r, &g, &b, &a);
            ar += 1 * r;
            ag += 1 * g;
            ab += 1 * b;
            aa += 1 * a;
            SDL_GetRGBA(src[y1 * pitch32 + x0], fmt, &r, &g, &b, &a);
            ar += 2 * r;
            ag += 2 * g;
            ab += 2 * b;
            aa += 2 * a;
            SDL_GetRGBA(src[y1 * pitch32 + x1], fmt, &r, &g, &b, &a);
            ar += 4 * r;
            ag += 4 * g;
            ab += 4 * b;
            aa += 4 * a;
            SDL_GetRGBA(src[y1 * pitch32 + x2], fmt, &r, &g, &b, &a);
            ar += 2 * r;
            ag += 2 * g;
            ab += 2 * b;
            aa += 2 * a;
            SDL_GetRGBA(src[y2 * pitch32 + x0], fmt, &r, &g, &b, &a);
            ar += 1 * r;
            ag += 1 * g;
            ab += 1 * b;
            aa += 1 * a;
            SDL_GetRGBA(src[y2 * pitch32 + x1], fmt, &r, &g, &b, &a);
            ar += 2 * r;
            ag += 2 * g;
            ab += 2 * b;
            aa += 2 * a;
            SDL_GetRGBA(src[y2 * pitch32 + x2], fmt, &r, &g, &b, &a);
            ar += 1 * r;
            ag += 1 * g;
            ab += 1 * b;
            aa += 1 * a;
...
// Tests if a pixel is considered black using a simple threshold.
...
// Dilates black pixels using a 3x3 neighborhood.
...
// Erodes black pixels using a 3x3 neighborhood.
...
// Performs morphological closing (dilate then erode) on black pixels.
void close_black3x3(SDL_Renderer* renderer, SDL_Surface* master_surface,
                    SDL_Texture** window_output, int iterations)
{
    if (!renderer || !master_surface || !window_output)
...
// Sorts 9 values in ascending order.
...
// Applies a 3x3 median filter to reduce noise.
void median3x3(SDL_Renderer* renderer, SDL_Surface* master_surface,
               SDL_Texture** window_output)
{
    if (!renderer || !master_surface || !window_output)
...
// Checks if a pixel is below a threshold (black) using red channel.
...
// Dilates black pixels using a cross-shaped neighborhood.
static void dilate_black_cross_thresh(SDL_Surface* s, Uint32* src, Uint32* dst,
                                      Uint8 thr)
{
    const int w = s->w, h = s->h;
    const int pitch32 = s->pitch / 4;
    ...
    for (int y = 0; y < h; y++)
    {
        for (int x = 0; x < w; x++)
        {
            ...
            if (!any && y > 0)
                any |= is_black_px(fmt, src[(y - 1) * pitch32 + x], thr);
            if (!any && y + 1 < h)
                any |= is_black_px(fmt, src[(y + 1) * pitch32 + x], thr);

            if (!any && x > 0)
                any |= is_black_px(fmt, src[y * pitch32 + (x - 1)], thr);
            if (!any && x + 1 < w)
                any |= is_black_px(fmt, src[y * pitch32 + (x + 1)], thr);
...
// Erodes black pixels using a cross-shaped neighborhood.
static void erode_black_cross_thresh(SDL_Surface* s, Uint32* src, Uint32* dst,
                                     Uint8 thr)
{
    const int w = s->w, h = s->h;
    const int pitch32 = s->pitch / 4;
    ...
    for (int y = 0; y < h; y++)
    {
        for (int x = 0; x < w; x++)
        {
            ...
            if (y > 0)
                all &= is_black_px(fmt, src[(y - 1) * pitch32 + x], thr);
            if (y + 1 < h)
                all &= is_black_px(fmt, src[(y + 1) * pitch32 + x], thr);
            if (x > 0)
                all &= is_black_px(fmt, src[y * pitch32 + (x - 1)], thr);
            if (x + 1 < w)
                all &= is_black_px(fmt, src[y * pitch32 + (x + 1)], thr);
...
// Tries to reconnect thin strokes using a small morphological close.
void repair_strokes(SDL_Renderer* renderer, SDL_Surface* master_surface,
                    SDL_Texture** window_output)
{
    if (!renderer || !master_surface || !window_output)
...
// Returns the grayscale value for a pixel.
...
// Blurs a grayscale image using an integral image box blur.
static void box_blur_gray_integral(const Uint8* g, int w, int h, int r,
                                   Uint8* out)
{
    const int W1 = w + 1;
...
// Applies Sauvola thresholding on a grayscale buffer to produce black/white.
static void sauvola_on_gray(const Uint8* g, int w, int h, int radius, double k,
                            double R, int bias, Uint8* out_bw)
{
    const int W1 = w + 1;
    size_t N = (size_t)(w + 1) * (size_t)(h + 1);
...
// Computes the ratio of black pixels in a binary image.
...
// Computes mean and standard deviation of an array of bytes.
...
// Binarizes the image using background removal and Sauvola thresholding.
void binarize(SDL_Renderer* renderer, SDL_Surface* master_surface,
              SDL_Texture** window_output)
{
    if (!renderer || !master_surface || !window_output)
