...
static HelpItem HELP_ITEMS[] = {
    {"load",      "load <file>\nLoads an image into master_surface.\nExample: load input.png"},
    {"rotate",    "rotate <deg>\nRotates the current image by <deg> degrees.\nExample: rotate 90"},
    {"autorotate","autorotate\nEstimates and applies the rotation angle automatically."},
    {"grayscale", "grayscale\nConverts the image to grayscale."},
    {"binarize",  "binarize\nConverts the image to black/white using adaptive thresholding."},
    ...
    {"auto",      "auto\nRuns the full pipeline: preprocess, rotate, binarize, boxes, export."},
...
static void cmdwindow_logger(void *userdata, const char *msg)
{
    ...
    if (!gcmd.logbuf) return;

    size_t len = strlen(msg);
    if (gcmd.loglen + len + 1 >= gcmd.logcap) {
        gcmd.loglen = 0; // reset if full
    }
...
static void render_wrapped(const char *text, int x, int y, int wrapWidth, int *outHeight)
{
    if (outHeight) *outHeight = 0;
    if (!text || !*text) return;

    // Use wrapped rendering (SDL_ttf)
    SDL_Surface *surf = TTF_RenderText_Blended_Wrapped(gcmd.font, text, gcmd.textColor, (Uint32)wrapWidth);
    if (!surf) return;
...
static void cmdwindow_render_learn_form(void)
{
    ...
    SDL_SetWindowPosition(gcmd.window, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED);

    SDL_SetRenderDrawColor(gcmd.renderer, 20, 20, 20, 255);
    SDL_RenderClear(gcmd.renderer);

    render_line("LEARN (Tab/Up/Down = change field, type = edit, Enter = run, Esc = cancel)", 10, 10);
...
static void cmdwindow_render_help_list(void)
{
    ...
    for (int i = 0; i < HELP_COUNT; i++) {
        char line[160];
        snprintf(line, sizeof(line), "%c %s", (i == gcmd.helpIndex ? '>' : ' '), HELP_ITEMS[i].name);
        render_line(line, 20, y);
        y += 22;
    }
...
static void cmdwindow_exit_help_to_input(void)
{
    ...
    // restore default cmd window size
    SDL_SetWindowSize(gcmd.window, CMD_WINDOW_WIDTH, CMD_WINDOW_HEIGHT);
    SDL_SetWindowPosition(gcmd.window, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED);
...
int cmdwindow_init(void)
{
    ...
    gcmd.loglen = 0;
    if (gcmd.logbuf) gcmd.logbuf[0] = '\0';
...
void cmdwindow_quit(void)
{
    ...
    if (gcmd.logbuf) free(gcmd.logbuf);
    SDL_StopTextInput();
}
...
CmdResult cmdwindow_handle_event(SDL_Event *e)
{
    ...
    if (gcmd.mode == CMD_MODE_LEARN_LOG)
    {
        if (e->type == SDL_KEYDOWN) {
            ...
            int step = (gcmd.log_line_height > 0 ? gcmd.log_line_height : 18) * 3;

            if (k == SDLK_UP) {
                gcmd.log_scroll -= step;
                if (gcmd.log_scroll < 0) gcmd.log_scroll = 0;
                cmdwindow_render_learn_log();
                return res;
            }
            if (k == SDLK_DOWN) {
    ...
    // HELP MODE: consume keys
    if (gcmd.mode == CMD_MODE_HELP_LIST || gcmd.mode == CMD_MODE_HELP_DESC) {
        if (e->type == SDL_KEYDOWN) {
            SDL_Keycode k = e->key.keysym.sym;

            if (k == SDLK_ESCAPE) {
                if (gcmd.mode == CMD_MODE_HELP_DESC) cmdwindow_back_to_help_list();
                else cmdwindow_exit_help_to_input();
                return res;
            }

            if (gcmd.mode == CMD_MODE_HELP_LIST) {
                if (k == SDLK_UP) {
                    gcmd.helpIndex = (gcmd.helpIndex - 1 + HELP_COUNT) % HELP_COUNT;
                    cmdwindow_render_help_list();
                    return res;
                }
                if (k == SDLK_DOWN) {
    ...
    if (gcmd.mode == CMD_MODE_LEARN_FORM) {
        ...
        char *field =
            (gcmd.learn_field == 0) ? gcmd.learn_dataset :
        ...
        size_t fieldCap =
            (gcmd.learn_field <= 1) ? 255 :
            (gcmd.learn_field == 2) ? 31 :
            (gcmd.learn_field == 3) ? 31 : 31;
        ...
        if (e->type == SDL_KEYDOWN) {
            ...
            if (k == SDLK_RETURN || k == SDLK_KP_ENTER) {
                ...
                if (gcmd.learn_dataset[0] == '\0' || gcmd.learn_out[0] == '\0') {
                    printf("learn: please fill Dataset file and Output model\n");
                    return res;
                }

                // Build a raw command that main() can parse
                ...
                // leave learn mode back to input UI
                gcmd.mode = CMD_MODE_LEARN_LOG;
                gcmd.log_scroll = 0;
                if (gcmd.logbuf) { gcmd.loglen = 0; gcmd.logbuf[0] = '\0'; }
                cmdwindow_render_learn_log();
                return res; // emits learn command to main (so main calls train)
            }
        }
    ...
    if (e->type == SDL_TEXTINPUT) {
        ...
        if (e->key.keysym.sym == SDLK_BACKSPACE) {
            ...
            // enter help mode if requested
            if (strcmp(res.operation, "help") == 0 || strcmp(res.operation, "man") == 0) {
                gcmd.input[0] = '\0';
                cmdwindow_enter_help();
                return (CmdResult){0}; // do not emit a command to main app
            }
            ...
            if (strcmp(res.operation, "learn") == 0) 
            {
                ...
                return (CmdResult){0}; // do not emit to main
            }
...
static void cmdwindow_render_learn_log(void)
{
    ...
    const char *txt = (gcmd.logbuf && gcmd.logbuf[0]) ? gcmd.logbuf : "(no output yet)";

    // Render full wrapped text to a surface
    ...
    if (gcmd.log_scroll < 0) gcmd.log_scroll = 0;
    if (gcmd.log_scroll > maxScroll) gcmd.log_scroll = maxScroll;

    SDL_Rect src = { 0, gcmd.log_scroll, texW, viewH };
    if (src.h > texH - src.y) src.h = texH - src.y;
