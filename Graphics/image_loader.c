...
void load_image(SDL_Surface** surface_out, const char* file)
{
    ...
    if (!raw)
        errx(EXIT_FAILURE, "load_image: SDL_LoadBMP failed: %s",
             SDL_GetError());
    ...
    if (!conv)
        errx(EXIT_FAILURE, "load_image: SDL_ConvertSurfaceFormat failed: %s",
             SDL_GetError());
...
void create_texture_from_surface(SDL_Texture** texture_out,
...
void render_texture_rotated(SDL_Renderer* renderer, SDL_Texture* texture,
                            SDL_Texture* master_texture, int maxw, int maxh,
                            int srcw, int srch, const double angle)
{
    SDL_Texture* temp = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888,
                                          SDL_TEXTUREACCESS_TARGET, maxw, maxh);
    ...
    dst.x = (maxw - srcw) / 2;
    dst.y = (maxh - srch) / 2;
    int err = SDL_RenderCopyEx(renderer, texture, NULL, &dst, angle, NULL,
                               SDL_FLIP_NONE);
    if (err)
    {
