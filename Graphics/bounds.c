...
void flood_fill(SDL_Surface** surface, int x, int y, int** visited,
                Coord** blob, int* blob_size)
{
    int w = (*surface)->w;
    int h = (*surface)->h;
    if (x < 0 || y < 0 || x >= w || y >= h)
...
// main function of the blob finding algorithm, reads surface pixels, when black
// pixel is encountered run flood_fill to visit all the blob, visited pixels are
// later ignored.
Coord** find_blobs_rec(SDL_Surface** surface, int* blob_count,
                       int** blob_sizes_out)
{

    if (!surface || !blob_count || !blob_sizes_out)
        return NULL;
    ...
    for (int y = 0; y < h; y++)
    {
        for (int x = 0; x < w; x++)
        {
            ...
            if (r < 10 && g < 10 && b < 10)
            {
                ...
                flood_fill(surface, x, y, visited, &res[*blob_count - 1],
        ...
        for (int x = 0; x < w; x++)
        {
            ...
            if (r < 10 && g < 10 && b < 10)
            {
                ...
                for (int i = 0; i < *blob_count && !added; i++)
                {
                    for (int j = 0; j < blob_sizes[i]; j++)
                    {
                        if (is_adjacent(p, res[i][j]))
                        {
                            blob_sizes[i]++;
                            res[i] =
                                realloc(res[i], blob_sizes[i] * sizeof(Coord));
                ...
                if (!added)
                {
                    ...
                    res = realloc(res, (*blob_count) * sizeof(Coord*));
                    blob_sizes =
                        realloc(blob_sizes, (*blob_count) * sizeof(int));
...
// foreach blob gives a corresponding Box, a Box being determined by the
// up-left-most and down-right-most pixels of a blob.
Box* compute_blob_boxes(Coord** blobs, int* blob_sizes, int blob_count)
{
    if (!blobs || !blob_sizes || blob_count <= 0)
        return NULL;
...
void draw_boxes(SDL_Surface** surface, Box* boxes, int count, Uint8 r, Uint8 g,
                Uint8 b)
{
    // --- DEBUG TESTS ---
    printf("\n[draw_boxes] called\n");
    ...
    {
        printf("  ERROR: draw_boxes assumes 32bpp (4 bytes per pixel) but "
               "surface is %d Bpp!\n",
               s->format->BytesPerPixel);
    }
    ...
    {
        printf(
            "  WARNING: pitch (%d) not divisible by 4, check stride logic.\n",
            s->pitch);
    }
...
// compared ycenter of box a and b if |centera.y - centerb.y| < max_diff then we
// consider they are likely on the same row (of course for reasonably low value
// of max_y_diff)
...
// Calls the differentiation for boxes algorithm and appends results to boxes
// array
...
// Main algorithm to differentiate between the list, the grid and the words in
// the list using guesses about the disposition and way these parts are split.
// this assumes the list of words is to the side of the grid and wouldn't work
// on a differnet disposition, say the list is on top of the grid.
Box* differentiate_grid_list_and_words(Box* sorted, int n, int* out_count)
{
    ...
    {
        grid_boxes = left_boxes;
        gcount = left_count;
        list_boxes = right_boxes;
        lcount = right_count;
    }
    else
    {
        grid_boxes = right_boxes;
        gcount = right_count;
        list_boxes = left_boxes;
        lcount = left_count;
    }
