...
void resize_nearest_gray(const uint8_t *src, int src_w, int src_h,
...
void load_bmp_as_vector(const char *filename, int target_w, int target_h, double *out_vec)
{
	...
	{
		fclose(f);
		errx(EXIT_FAILURE,"load_bmp_as_vector() file format error : %s is not a bmp file\n"
				, filename);
	}
	...
	{
		fclose(f);
		errx(EXIT_FAILURE,"Unsupported BMP format (%s): bpp=%d, compression=%u\n", filename, biBitCount, biCompression);
	}
	...
	{
		fclose(f);
		errx(EXIT_FAILURE, "Only 8, 24 or 32-bit BMP supported (%s has %d bpp)\n", filename, biBitCount);
	}
	...
	for (int y = 0; y < height; ++y) 
	{
		if (fread(row, 1, row_padded, f) != (size_t)row_padded) 
		{
			...
			free(row);
			fclose(f);
			errx(EXIT_FAILURE, "load_bmp_as_vector() Error reading pixel data from %s\n", filename);
		}
