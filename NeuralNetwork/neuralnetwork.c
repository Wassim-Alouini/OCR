...
//generations de valeurs aleatoires deterministes
...
NeuralNetwork *init_nn(int inp, int h, int o, double lr)
{
	...
	nn->o_2 = o;
	nn->lr = lr; 
	nn->w_0_1 = init_m(inp, h);          //allocation dynamique des poids et des biais
	nn->w_1_2 = init_m(h, o);       
	nn->b_1 = malloc(h * sizeof(double));
	nn->b_2 = malloc(o * sizeof(double));
	
	for (int i = 0; i < inp; i++)    //initialisations des biais et des poids
	{
		for (int j = 0; j < h; j++)
		{
...
//calcul des sortiess en fonctions des entrees input
...
//retropropagation: 1 epoch = 4 retroprop : 1 pour chaque input possible
void SGD(NeuralNetwork *nn, double inputs[][2], double labels[][1], int data, int epochs) 
{
	for (int epoch = 0; epoch < epochs; epoch++)
	{
		...
		for (int s = 0; s < data; s++) 
		{
			...
			//calcul de l'erreur locale de chaque neurone 
			double grad_o_2[1];
			...
			//mise a jour des poids en foncition de l'erreur locale et du learning rate
			for (int j = 0; j < nn->h_1; j++)
			{
				for (int k = 0; k < nn->o_2; k++) 
				{
		...
		//suivi de la convergence toute les 1000 iterations 
