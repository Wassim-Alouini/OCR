...
// training loop
void SGD(NeuralNetwork* nn, Dataset* ds, int epochs, double lr, LogFn logger,
...
NeuralNetwork* nn_load(const char* filename)
{
    ...
    for (int i = 0; i < nn->inp_0; ++i)
    {
        for (int j = 0; j < nn->h_1; ++j)
        {
            e = fscanf(f, "%lf", &nn->w_0_1[i][j]);
        }
    }

    for (int k = 0; k < nn->o_2; ++k)
    {
        e = fscanf(f, "%lf", &nn->b_2[k]);
    }
           ...
           double lr, int hidden_size, LogFn logger, void* logger_userdata)
{
    ...
    nn_logf(logger, logger_userdata,
            "Training parameters :\n hidden = %d\nepochs = %d\nlr = %f \n",
